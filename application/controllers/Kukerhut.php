...
class Kukerhut extends CI_Controller
{
    ...
    public function products()
    {
        ...
        $config['base_url'] = base_url('kukerhut/products'); //site url
    ...
    public function category()
    {
        ...
        // //konfigurasi pagination
        $config['base_url'] = base_url('kukerhut/category/') . $id; //site url
        $config['total_rows'] = $this->db->count_all('produk'); //total row
        $config['per_page'] = $config['total_rows'];  //show record per halaman
        $config["uri_segment"] = 3;  // uri parameter
        $choice = $config["total_rows"] / $config["per_page"];
        $config["num_links"] = floor($choice);
